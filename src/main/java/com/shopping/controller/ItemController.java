...
public class ItemController {
    ...
    public String itemDtl(@PathVariable("itemId") Long itemId, Model model){
       /* Long itemId = Long.parseLong(request.getParameter("itemId"));
          @PathVariable("itemId") Long itemId와 같은 의미
       */
        try{
    ...
    public String itemUpdate(@Valid ItemFormDto itemFormDto, BindingResult bindingResult, @RequestParam("itemImgFile")List<MultipartFile> itemImgFileList, Model model){
        ...
        //  bindingResult : 폼 유효성검사 시 문제가 있는지 체크 하기위한 클래스
        //  itemImgFileList : 폼에서 넘오는 여러 개의 <input type="file"> 객체 리스트
        //  model : 뷰 영역으로 넘겨줄 정보들을 바인 하기위한 모델 객체
        ...
        if(itemImgFileList.get(0).isEmpty() && itemFormDto.getId() == null){
            model.addAttribute("errorMessage", "첫번재 상품 이미지는 필수 입력사항 입니다.");
            return whenError;
        }
        try {
